...
/* --- অডিও এবং ভিডিও কল সিস্টেম (WebRTC) --- */
async function startCall(type) {
    if (!activeChatReceiverId) return;
    currentCallType = type;
    activeCallPartnerId = activeChatReceiverId;
    ...
    const localVideo = document.getElementById("localVideo");
    const remoteVideo = document.getElementById("remoteVideo");
    const partnerName = document.getElementById("chatReceiverName")?.innerText || "User";
...
async function acceptIncomingCall() {
    ...
    const remoteVideo = document.getElementById("remoteVideo");
    const callRoomId = getChatRoomId(auth.currentUser.uid, incomingCallerId);
    const partnerName = document.getElementById("incomingCallerName")?.innerText || "User";
    document.getElementById("callPartnerName").innerText = partnerName;
